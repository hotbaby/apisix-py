...
class TestConsumer(unittest.TestCase):
    ...
    def test_list_consumers(self):
        resp = self.consumer_api.list_consumers()
        assert resp['code'] == 0
        print(resp)

    def test_retrieve_consumer(self):
        consumers = self.consumer_api.list_consumers()
        consumer = consumers['data']['rows'][0]

        resp = self.consumer_api.retrieve_consumer(consumer['username'])
        print(resp)
        assert resp['code'] == 0
    ...
    def test_update_consumer(self):
        ...
        resp = self.consumer_api.update_consumer(consumer['username'], consumer)
        print(resp)
        assert resp['code'] == 0
